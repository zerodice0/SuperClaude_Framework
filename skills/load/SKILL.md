---
name: load
display_name: "Project Context Loading"
...
# Intent Detection
intents:
  primary: ["load {project}", "load context", "restore session", "activate project", "initialize {session}"]
  keywords: [load, context, session, restore, activate, project, checkpoint, memory, persistence, initialize]
  patterns: ["^(load|restore|activate|initialize) (?P<target>(project|context|session|checkpoint|memory))?( (?P<path>.+))?$"]
  contexts: [session_management, project_activation, context_loading, persistence]
...
arguments:
  - name: target
    type: string
    required: false
    description: "Project path or checkpoint to load"
    infer_from: user_query

  - name: type
    type: enum
    values: [project, config, deps, checkpoint]
    required: false
  ...
  - name: refresh
    type: bool
    required: false
    description: "Refresh cached context"
    infer_from: user_query
    default: false
...
tags: [session, loading, context, persistence, serena]
---
