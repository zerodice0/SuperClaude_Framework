---
name: select-tool
display_name: "MCP Tool Selection"
...
# Intent Detection
intents:
  primary: ["select tool {operation}", "choose mcp {task}", "which tool {operation}", "optimal tool {task}"]
  keywords: [select, tool, mcp, choose, optimal, serena, morphllm, routing, selection, decision]
  patterns: ["^(select|choose|which|optimal) tool (for )?(?P<operation>.+)$"]
  contexts: [tool_selection, mcp_routing, optimization]
...
arguments:
  - name: operation
    type: string
    required: true
    description: "Operation requiring tool selection"
    infer_from: user_query

  - name: analyze
    type: bool
    required: false
    description: "Show analysis reasoning"
    infer_from: user_query
    default: false
  ...
  - name: explain
    type: bool
    required: false
    description: "Explain selection decision"
    infer_from: user_query
    default: false
...
tags: [mcp, tool-selection, routing, optimization, serena, morphllm]
---
