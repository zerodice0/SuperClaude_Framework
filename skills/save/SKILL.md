---
name: save
display_name: "Session Context Persistence"
...
# Intent Detection
intents:
  primary: ["save {session}", "save context", "persist session", "create checkpoint", "preserve {progress}"]
  keywords: [save, persist, checkpoint, context, session, memory, preserve, store, backup, archive]
  patterns: ["^(save|persist|preserve|checkpoint|backup|archive) (?P<target>(session|context|progress|checkpoint|memory))?$"]
  contexts: [session_management, persistence, checkpoint, memory_storage]

# Arguments
arguments:
  - name: type
    type: enum
    values: [session, learnings, context, all]
    required: false
  ...
  - name: checkpoint
    type: bool
    required: false
    description: "Create recovery checkpoint"
    infer_from: user_query
    default: false
...
tags: [session, persistence, checkpoint, context, serena]
---
