---
name: reflect
display_name: "Task Reflection and Validation"
...
# Intent Detection
intents:
  primary: ["reflect {task}", "validate {completion}", "assess {progress}", "review {work}", "evaluate {session}"]
  keywords: [reflect, reflection, validate, validation, assess, review, evaluate, progress, quality, completion, adherence]
  patterns: ["^(reflect|validate|assess|review|evaluate)( on)?( (?P<target>(task|work|progress|session|completion)))?$"]
  contexts: [reflection, validation, assessment, quality_review]

# Arguments
arguments:
  - name: type
    type: enum
    values: [task, session, completion]
    required: false
...
tags: [reflection, validation, assessment, quality, serena]
---
