...
# Intent Detection
intents:
  primary: ["index {project}", "generate index", "create project docs", "organize documentation", "build knowledge base"]
  keywords: [index, documentation, organize, catalog, reference, knowledge-base, project-docs, api-docs, structure]
  patterns: ["^(index|catalog|organize) (?P<target>.+)?$", "^(generate|create|build) (index|docs|documentation|knowledge.base)( for (?P<target>.+))?$"]
  contexts: [documentation, indexing, organization, knowledge_management]
...
arguments:
  - name: target
    type: string
    required: false
    description: "Target directory to index"
    infer_from: user_query
    default: "."

  - name: type
    type: enum
    values: [docs, api, structure, readme]
    required: false
...
tags: [documentation, indexing, organization, knowledge-base]
---
